...
// =================================================================
// FUNÇÃO PRINCIPAL: CARREGAR TODAS AS VAGAS (COM FILTRO NO FIREBASE)
// =================================================================
...
    try {
        // CORREÇÃO ESSENCIAL: Adicionando where('status') e orderBy('titulo') 
        // para atender a regra de segurança e forçar a criação do índice composto necessário.
...
        vagasContainer.innerHTML = '<p class="error-message" style="color: white;">Erro ao carregar as vagas. Verifique suas regras de segurança do Firestore e os índices necessários.</p>';
    }
};
...
        // Se estiver na VagasAluno.html, carrega as vagas
        if (path.includes('VagasAluno.html')) loadAvailableJobs().then(setupFilterListeners);
        
        // REMOVIDO: O bloco que causava o erro em minhasCandidaturas.html
        
    } else {
        const protectedPaths = ['VagasAluno.html', 'minhasCandidaturas.html', 'PerfilAluno.html'];
        const currentPage = window.location.pathname.split('/').pop();
        // Redireciona para o index (ou login) se tentar acessar páginas protegidas
