    ...
    db.collection('vagas')
      ...
      .then(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                // 💡 INÍCIO DA MUDANÇA: Renderização do Período
                const periodo = vaga.periodo ? `<p class="job-period">Período: ${vaga.periodo}</p>` : '';
                // 💡 FIM DA MUDANÇA
                ...
                vagaCard.innerHTML = `
                    ...
                    ${cursos}
                    ${periodo} <p class="job-time">Carga Horária: ${vaga.cargaHoraria}</p>
...
const setupCreateJobForm = () => {
    ...
    if (createJobForm) {
        createJobForm.addEventListener('submit', (e) => {
            ...
            // ⚠️ NOTA: Assumindo que 'periodo' é capturado no HTML da página de criação com ID 'periodo'.
            const vagaData = {
                titulo: document.getElementById('titulo').value,
                descricao: document.getElementById('descricao').value,
                requisitos: document.getElementById('requisitos').value,
                cargaHoraria: document.getElementById('cargaHoraria').value,
                periodo: document.getElementById('periodo') ? document.getElementById('periodo').value : '', // Caputa o período (se o elemento existir)
...
const setupJobActions = () => {
        ...
        if (targetButton.classList.contains('edit-btn') && editModal) {
             db.collection('vagas').doc(vagaId).get().then(doc => {
                 if (doc.exists) {
                     ...
                     // 💡 INÍCIO DA MUDANÇA: Pré-preenchimento do Período no Modal
    ...
    if (editForm) {
        editForm.addEventListener('submit', (e) => {
            ...
            // 💡 INÍCIO DA MUDANÇA: Captura do Período Editado
            const periodoEditado = document.getElementById('edit-periodo') ? document.getElementById('edit-periodo').value : undefined;
            // 💡 FIM DA MUDANÇA
            ...
            const updatedData = {
                 ...
                 periodo: periodoEditado, // 💡 APLICADO: Salva o Período Editado
